...
{
	int retcode;
	int xmin,xmax,ymin,ymax,x_max=MOUS_X_MAX,y_max=MOUS_Y_MAX;//
	int size;
...
void newmouse(Mouse *m){
	int *nx = &(m->posx),*ny = &(m->posy),*nclick = &(m->click);
	int x0 = *nx,y0 = *ny,click0 = *nclick;
	int xn,yn,clickn;
	mread(&xn,&yn,&clickn);
	m->posx = xn;
	m->posy = yn;
...
int ismouse_clickbt(Mouse mos,int x1,int y1,int x2,int y2){
	if( x1<mos.posx && mos.posx>x2&& y1<mos.posy && mos.posy<y2 && mos.click ==1){
		return 1;
	}
	else{
		return 0;
